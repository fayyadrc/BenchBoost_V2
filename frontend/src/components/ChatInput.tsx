        ...
        >
            ...
            >
                <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4">
                    <MessageSquare className={`w-4 sm:w-5 h-4 sm:h-5 shrink-0 ${isDark ? 'text-white/40' : 'text-slate-400'}`} />
                    <textarea
                        className={`flex-1 bg-transparent border-none outline-none resize-none text-xs sm:text-sm font-medium max-h-32 ${isDark ? 'text-white placeholder-white/30' : 'text-slate-900 placeholder-slate-400'
                            }`}
                        value={input}
                    ...
                    <button
                        ...
                        className={`shrink-0 flex items-center justify-center w-9 sm:w-10 h-9 sm:h-10 border-2 transition-all ${input.trim() && !isLoading
                            ? isDark
                                ? 'text-white'
